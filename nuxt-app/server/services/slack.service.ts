...
import { TIMEOUTS, TIMEZONE } from '../utils/config'
import type { SlackBlock, SlackResponse } from '~/types/slack'
...
async function callApi(
  ...
  if (!authToken) {
    console.warn('[Slack] No token available for API call', { endpoint })
  ...
  try {
    ...
    if (!data.ok) {
      console.warn('[Slack] API returned error', { endpoint, error: data.error })
    }
    ...
    console.error('[Slack] API call failed', { endpoint, error: errorMessage })
...
export async function postMessage(
  ...
  if (!targetChannel) {
    console.error('[Slack] No channel specified for message')
    return { ok: false, error: 'No channel specified' }
  }

  console.info('[Slack] Posting message', { channel: targetChannel })
...
export async function postThreadReply(
  ...
  if (!targetChannel) {
    console.error('[Slack] No channel specified for thread reply')
    return { ok: false, error: 'No channel specified' }
  }

  console.info('[Slack] Posting thread reply', {
...
export async function updateMessage(
  ...
  if (!targetChannel) {
    console.error('[Slack] No channel specified for message update')
    return { ok: false, error: 'No channel specified' }
  }

  console.info('[Slack] Updating message', { channel: targetChannel, ts })
...
export async function getHistory(
  ...
  if (!targetChannel) {
    console.error('[Slack] No channel specified for history')
    return { ok: false, error: 'No channel specified' }
  }
...
export async function getThreadReplies(
  ...
  if (!targetChannel) {
    console.error('[Slack] No channel specified for thread replies')
    return { ok: false, error: 'No channel specified' }
  }
